---
title: "archaic_admixture"
author: "SoerenJoergensen"
date: "`r Sys.Date()`"
output: html_document
---

```{r setup, include=FALSE}
knitr::opts_chunk$set(echo = TRUE, fig.width = 14, fig.height = 5)
library(tidyverse)
library(cowplot)
library(knitr)
library(ggdendro)
theme_set(theme_bw(base_size = 12))
#setwd("~/Documents/git/popgen-project/")
```

## Project Plan for Exploring Non-African Archaic Segments

### 1. Data Exploration and Preprocessing

Load and preprocess the data files. We have to recalculate the lengths of the segments (VIP knowledge from Moi)

```{r data-initialization}

archaic_segments <- read_tsv("ArchaicAdmixture/ArchaicSegments.txt") %>% 
  mutate(length = end-start+1000) %>% 
  filter(length>0)

test_segments <- archaic_segments %>% slice_sample(n=10000) %>% mutate(chrom=factor(chrom, levels= c(1:22, 'X')))

 snps <- read_tsv("ArchaicAdmixture/SNPs.txt") %>%  
     mutate(origin = factor(
       case_when(
         archaic %in% c("AltaiNea_nofil","Vi33.19_nofil") ~ "Neanderthal",
         archaic == "Denisova_nofil" ~ "Denisova",
         .default = 'human'),
       levels = c('human', 'Neanderthal', 'Denisova')))
 
 test_snps <- snps %>% slice_sample(n=10000)
 
 test_snps %>% head() %>% kable
 
 length(unique(test_snps$start))
 
 archaic_snps <- snps %>% filter(archaic != "human") %>% select(-c("archaic"))
 
 nrow(archaic_snps)
 nrow(archaic_segments)
```


# 2. How correlated are the archaic contents in (any) two indiviuals

## Try to find shared segments between a pair of individuals

Maybe we can generalize later

```{r function-find-shared-segments}

# Function to find shared segments between two individuals
find_shared_segments <- function(individual_1, individual_2, segments_data) {
  
  df <- segments_data %>% filter(name %in% c(individual_1,individual_2)) %>% 
    mutate(origin = case_when(
        Shared_with_Altai > Shared_with_Denisova | Shared_with_Vindija > Shared_with_Denisova ~ "Neanderthal",
        Shared_with_Altai < Shared_with_Denisova & Shared_with_Vindija < Shared_with_Denisova ~ "Denisovan",
        Shared_with_Altai + Shared_with_Vindija + Shared_with_Denisova == 0 ~ "Unclassified"
    )) %>% select(name, chrom, start, end, origin)
  df1 <- df %>% filter(name==individual_1)
  df2 <- df %>% filter(name==individual_2)
  
  #cat("Comparing individuals: ", individual_1, ":", individual_2)
  
  shared_segments <- data.frame()
  
  for (chrom in c(1:22, 'X')) {
    #cat("Chromosome:", chrom, "\t")
    
    df1_chrom <- df1 %>% filter(chrom == !!chrom)
    df2_chrom <- df2 %>% filter(chrom == !!chrom)
    
    overlaps <- fuzzyjoin::interval_inner_join(df1_chrom, df2_chrom, c("start", "end")) %>% 
      filter((origin.x == origin.y) & (origin.x %in% c("Neanderthal", "Denisova")))
    print(overlaps)

    overlaps <- overlaps %>% mutate(
      overlap_length = case_when(nrow(overlaps)>0 ~ pmin(end.x, end.y) - pmax(start.x, start.y),
                                 .default = 0))
    
    
    #cat("--> length:", sum(overlaps$overlap_length), "\n")
    
    shared_segments <- rbind(shared_segments, overlaps)
  }
  
  shared_segments <- shared_segments %>% mutate(chrom=chrom.x, origin=origin.x) %>% select(name.x, name.y, chrom, start.x, start.y, end.x, end.y, origin, overlap_length)
  
  #cat("\t total overlap: ", sum(shared_segments$overlap_length), "\n")
  
  return(shared_segments )
}

# Test function 
  find_shared_segments("S_Hungarian-1", "ERS700536", archaic_segments)
 
# # Test validity 
# find_shared_segments("ERS700536", "ERS700536", test_segments) %>% 
#     summarise(total=sum(overlap_length))


# test_segments %>% filter(name=="ERS700536") %>% summarise(total=sum(length))

```

```{r use-function, eval=FALSE}

# Example: Calculate shared SNPs between two individuals
individual_1 <- sample(unique(test_segments$name), size = 1)
individual_2 <- sample(unique(test_segments$name), size = 1)
shared_segments <- find_shared_segments(individual_1, individual_2, test_segments)
shared_segments

```

```{r use-the-cluster, eval=FALSE}

# Get all unique individuals
individuals <- unique(test_segments$name)

# Define a function to be applied to each pair
find_shared_segments_for_pair <- function(pair) {
  shared_segments <- find_shared_segments(pair[1], pair[2], archaic_segments)
  
  # Calculate the total length of overlapping segments
  total_length <- sum(shared_segments$overlap_length)
  
  # Return a data frame with the results
  return(data.frame(individual_1 = pair[1], 
                    individual_2 = pair[2], 
                    total_length = total_length))
}

# Generate all possible pairs of unique individuals and apply the function
library(future)
library(furrr)


plan(cluster)
t0 <- Sys.time()
1:ncol(pairs[,1:20]) %>%
  future_map_dfr(\(i) find_shared_segments_for_pair(pairs[,i]), .progress = TRUE, .options = furrr_options(stdout = TRUE))

t1 <- Sys.time()
cat("took", t1-t0, "sec")

# results <- combn(individuals, 2, FUN = find_shared_segments_for_pair, simplify = FALSE)

# # Combine the results into a single data frame
# results <- do.call(rbind, results)

# # Write the results to a CSV file
# write.csv(results, file = "results.csv", row.names = FALSE)

```

```{r get-the-overlaps-computed-on-cluster}

pairwise_overlap <- read_csv("results_epas_x.csv")

archaic_individuals <- archaic_segments %>% 
  group_by(name) %>% 
  mutate(total_archaic = sum(length)) %>% 
  ungroup %>% 
  distinct(name, .keep_all = TRUE) %>% 
  select(name, pop, country, region, total_archaic)

pairwise_overlap <- pairwise_overlap %>%
  left_join(archaic_individuals, by = c("individual_1" = "name")) %>%
  rename(pop_1 = pop, country_1 = country, region_1 = region) %>% 
  left_join(archaic_individuals, by = c("individual_2" = "name")) %>%
  rename(pop_2 = pop, country_2 = country, region_2 = region)
  
pairwise_overlap <- pairwise_overlap %>% 
 mutate(individual_1=factor(individual_1),
        individual_2=factor(individual_2))

```

```{r make-the-heatmap}
reversed <- pairwise_overlap %>%
  select(c(2,1), everything()) %>% 
  rename(individual_2=individual_1, individual_1=individual_2) %>% 
  arrange(individual_1)

pairwise_overlap <- bind_rows(pairwise_overlap, reversed)

label_x <- pairwise_overlap %>% 
  group_by(region_1) %>% 
  summarise(n=n()) %>% 
  mutate(cumm = cumsum(n))


heatmap_plot <- pairwise_overlap %>% 
  arrange(region_1) %>% 
  ggplot(aes(x=individual_1, y=individual_2, fill=total_length)) +
  geom_tile() +
  scale_fill_gradient(low="white", high="red") +
  theme(#axis.text.x = element_text(size=6), #element_text(angle = 90, size=5),
        axis.text.y = element_blank(), #element_text(size=5),
        ) +
  scale_x_discrete(breaks=label_x$cumm, labels=label_x$region_1)
heatmap_plot

```

```{r try-using-complex-heatmap}

library(ComplexHeatmap)
library(circlize)

cols <- RColorBrewer::brewer.pal(length(unique(pairwise_overlap$region_1)), "RdYlGn")

individual_order <- archaic_individuals %>% arrange(factor(region), factor(pop))

compmap_df <- pairwise_overlap %>%
  arrange(region_1) %>% 
  select(c(1,2,3)) %>% 
  pivot_wider(names_from = individual_2, values_from = shared_x, values_fill = NA) %>%
  column_to_rownames("individual_1") %>% 
  as.matrix()

unique(pairwise_overlap$region_1)

column_clusters <- pairwise_overlap %>% distinct(individual_1, .keep_all = TRUE) %>% 
  select(individual_1,region_1) %>% as.data.frame()

row_clusters <- pairwise_overlap %>% distinct(individual_2, .keep_all = TRUE) %>% 
  select(individual_2,region_2) %>% as.data.frame()

bottom_annotation <- HeatmapAnnotation(foo = anno_block(
  gp = gpar(fill = cols),
  labels = factor(unique(column_clusters$region_1)), 
  labels_gp = gpar(col = "black", fontsize = 10)
))

left_annotation <- rowAnnotation(bar = anno_block(
  gp = gpar(fill = cols),
  labels = factor(unique(row_clusters$region_2)), 
  labels_gp = gpar(col = "black", fontsize = 10)
))
  
compmap <- Heatmap(compmap_df, name = "Total overlap",
                   show_column_dend = TRUE, show_row_dend = FALSE,
                   show_column_names = FALSE, show_row_names = FALSE,
                   row_order = individual_order$name,
                   column_order = individual_order$name
                   #bottom_annotation = bottom_annotation,
                   #left_annotation = left_annotation
                   )
compmap

```


```{r do-some-summary-statistics-on-correlation}

overlap_pair_df <- read_csv("results_diff.csv")

overlap_pair_df <- overlap_pair_df %>%
  left_join(archaic_individuals, by = c("individual_1" = "name")) %>%
  rename(pop_1 = pop, country_1 = country, region_1 = region) %>% 
  left_join(archaic_individuals, by = c("individual_2" = "name")) %>%
  rename(pop_2 = pop, country_2 = country, region_2 = region)

overlap_pair_df %>% group_by(region_1, region_2) %>% 
  summarise(mean=mean(2*total_length/(total_archaic.x+total_archaic.y))) %>% 
  ungroup() %>% 
  ggplot(aes(x=region_1, y=mean)) +
  geom_col(position="dodge") +
  facet_wrap(~region_2, nrow=1) +
  theme(
    axis.text.x = element_text(angle = 90)
  )

# Calculate the mean proportion of overlap between regions
overlap_pair_df %>% group_by(region_1, region_2) %>% 
  summarise(mean=mean(2*total_length/(total_archaic.x+total_archaic.y))) %>% 
  ungroup() %>% 
  rbind(., transform(., region_1=region_2, region_2=region_1)) %>% 
  ggplot(aes(x=region_1, y=region_2, fill=mean)) +
  geom_tile() +
  scale_fill_gradient2(low = "blue",
                       mid = "white",
                       high = "red") +
  theme(axis.text.x = element_text(angle=90, hjust = 1, vjust=0))

# Calculate the mean proportion of overlap between populations
overlap_pair_df %>% group_by(pop_1, pop_2) %>% 
  summarise(mean=mean(2*total_length/(total_archaic.x+total_archaic.y)),
            region=region_1[1]) %>% 
  ungroup() %>% 
  rbind(., transform(., pop_1=pop_2, pop_2=pop_1)) %>% 
  arrange(region) %>% 
  ggplot(aes(x=pop_1, y=pop_2, fill=mean)) +
  geom_tile() +
  scale_fill_gradient2(low = "blue",
                       mid = "white",
                       high = "red") +
  theme(axis.text.x = element_text(angle=90, hjust = 1, vjust=0))

```



## Calculate admixture proportion based on SNPs (region)

Calculate the correlation of archaic contents between individuals, or visualize to what extent individuals share SNPs:

```{r plot-the-counted-snps-derived-from-each-archaic-ancestry, fig.width=8 , eval=FALSE}

plot_admix_prop <- snps %>%
  group_by(name, region, origin) %>%
  summarize(shared_snps = n()) %>%  
  ungroup() %>% 
  group_by(name) %>% 
  reframe(region,
          origin, 
          shared_snps,
          shared_prop = shared_snps/(6e+09)
            ) %>% 
  arrange(origin) %>% 
  ggplot(aes(x=reorder(name, shared_snps), y=shared_prop, fill=origin)) +
  geom_bar(stat='identity', width=1) +
  scale_x_discrete(expand=c(0,0), name = "Individuals") +
  scale_y_continuous(expand=c(0,0), name = "Shared ancestry") +
  theme(
    axis.text.x = element_blank(),
    axis.ticks.x = element_blank()
  )
plot_admix_prop

```

```{r save-whole-plot, eval=FALSE}
ggsave(plot = plot_admix_prop, device = 'svg', 
       path = "graphics/", 
       filename = "admixture_proportion_whole.svg", width = 6.5, height = 2,
       dpi=2000)
```

> Now facet the plot 

```{r plot the counted snps derived from each archaic ancestry per region, eval=FALSE}

# Count the number of individuals in each facet group
facet_counts <- snps %>%
  group_by(region) %>%
  summarize(num_individuals = n_distinct(name))

# Make a labelling to be called in facet_wrap
include_n <- function(name) {
  number <- facet_counts$num_individuals[facet_counts$region == name]
  paste0(name, " (", number, ")")
}

plot_admix_prop_facet <- plot_admix_prop +
  facet_wrap(~region, scales='free_x', nrow = 1,
             labeller = as_labeller(include_n)) +
  theme(panel.spacing.x = unit(1, "mm"))
plot_admix_prop_facet

```

```{r save-facetted-plot, eval=FALSE}
ggsave(plot = plot_admix_prop_facet, device = 'svg', 
       path = "graphics/", 
       filename = "admixture_proportion_byorigin.svg", width = 6.5, height = 2,
       dpi=2000)

```

```{r save the combined plot, fig.height=10, eval=FALSE}

admix_proportion_combined <- plot_grid(plot_admix_prop, 
                                       plot_admix_prop_facet +  theme(legend.position = 'none'),
                                       labels = c('A','B'), nrow = 2)
admix_proportion_combined

```

```{r save-combined-plot, eval=FALSE}
ggsave(plot = plot_admix_prop_facet, device = 'svg', 
       path = "graphics/", 
       filename = "admixture_proportion_byorigin.svg", width = 6.5, height = 2,
       dpi=2000)
```

> Do the same thing on EPAS1


```{r}

snps_processed_epas <- snps %>%
  filter(chrom== '2', between(start, 46520806-500000, 46613842+500000)) %>% 
  group_by(name, region, origin, .drop = FALSE) %>%
  summarize(shared_snps = n()) %>%  
  ungroup() %>% 
  group_by(name) %>% 
  reframe(region,
          origin, 
          shared_snps,
          shared_prop = shared_snps/(46613842+500000-(46520806-500000))
            ) %>% 
  group_by(region, origin) %>% 
  arrange(region, origin, shared_prop, .by_group = TRUE) %>%
  ungroup()

plot_admix_prop_epas <- snps_processed_epas %>% 
  ggplot(aes(x=name, y=shared_prop, fill=origin)) +
  geom_bar(stat='identity', position='fill', width=1) +
  scale_x_discrete(expand=c(0,0), name = "Individuals") +
  scale_y_continuous(expand=c(0,0), name = "Shared ancestry") +
  theme(
    axis.text.x = element_blank(),
    axis.ticks.x = element_blank()
  )
plot_admix_prop_epas

plot_admix_prop_facet_epas <- plot_admix_prop_epas +
  facet_wrap(~region, scales='free_x', nrow = 1,
             labeller = as_labeller(include_n)) +
  theme(panel.spacing.x = unit(1, "mm"))
plot_admix_prop_facet_epas


```


> I want to know the mean proportions of ancestry in the whole data set

```{r table-mean-admixture-proportions, eval=FALSE}
mean_admix <- snps %>%
  group_by(name, region, origin) %>%
  summarize(shared_snps = n()) %>%  
  ungroup() %>% 
  group_by(name) %>% 
  reframe(region,
          origin, 
          shared_snps,
          shared_prop = shared_snps/sum(shared_snps)
          ) %>% 
  group_by(origin) %>% 
  summarise(full = mean(shared_prop)) %>% 
  pivot_longer(cols = 2, names_to="region",values_to = "mean_prop")

mean_admix

mean_admix_by_region <- snps %>%
  group_by(name, region, origin) %>%
  summarize(shared_snps = n()) %>%  
  ungroup() %>% 
  group_by(name) %>% 
  reframe(region,
          origin, 
          shared_snps,
          shared_prop = shared_snps/sum(shared_snps)
          ) %>% 
  group_by(origin,region) %>% 
  summarise(mean_prop = mean(shared_prop))

mean_admix <- bind_rows(mean_admix, mean_admix_by_region)

mean_admix %>% filter(region!="full") %>% 
  ggplot() +
  geom_col(aes(x=region, y=mean_prop, fill=origin), position='dodge') +
  geom_hline(data=mean_admix %>% filter(region=='full'), aes(yintercept = mean_prop, color=origin)) +
  theme(
    axis.text.x = element_text(angle=45, hjust=1)
  )

#mean_admix %>% pivot_wider(names_from=region, values_from = mean_prop) %>% kable(format = "latex", digits=3)

# I don't know of this is relevant, but here goes ANOVA one way
#aov(mean_prop~region, data=mean_admix_by_region) %>% summary(.)
#aov(mean_prop~region+origin, data=mean_admix_by_region) %>% summary(.)

```

> Get the admixture proportions in the EPAS1 region

```{r admixture-proportions-epas1,}

mean_admix_epas <- snps %>%
  filter(chrom== '2', between(start, 46520806-500000, 46613842+500000)) %>% 
  group_by(name, region, origin) %>%
  summarize(shared_snps = n()) %>%  
  ungroup() %>% 
  group_by(name) %>% 
  reframe(region,
          origin, 
          shared_snps,
          shared_prop = shared_snps/sum(shared_snps)
          ) %>% 
  group_by(origin) %>% 
  summarise(full = mean(shared_prop)) %>% 
  pivot_longer(cols = 2, names_to="region",values_to = "mean_prop")

mean_admix_epas

mean_admix_by_region_epas <- snps %>%
  filter(chrom== '2', between(start, 46520806-500000, 46613842+500000)) %>% 
  group_by(name, region, origin) %>%
  summarize(shared_snps = n()) %>%  
  ungroup() %>% 
  group_by(name) %>% 
  reframe(region,
          origin, 
          shared_snps,
          shared_prop = shared_snps/sum(shared_snps)
          ) %>% 
  group_by(origin,region) %>% 
  summarise(mean_prop = mean(shared_prop))

mean_admix_epas <- bind_rows(mean_admix_epas, mean_admix_by_region_epas)

mean_admix_epas %>% filter(region!="full") %>% 
  ggplot() +
  geom_col(aes(x=region, y=mean_prop, fill=origin), position='dodge') +
  geom_hline(data=mean_admix_epas %>% filter(region=='full'), aes(yintercept = mean_prop, color=origin)) +
  theme(
    axis.text.x = element_text(angle=45, hjust=1)
  )

mean_admix %>% pivot_wider(names_from=region, values_from = mean_prop) %>% kable(format = "latex", digits=3)

```

## Calculate admixture proportion based on SNPs (population)

```{r}
snps_w_pop <- snps %>% 
  left_join(archaic_individuals %>% select(name,pop), by=c("name" = "name")) %>% 
  mutate(pop=factor(pop, levels=unique(archaic_individuals$pop)))

admix_prop_pop <- snps_w_pop %>% 
  filter(chrom== '2', between(start, 46520806-500000, 46613842+500000)) %>% 
  group_by(name, pop, origin) %>%
  summarize(shared_snps = n()) %>%  
  ungroup() %>% 
  group_by(name) %>% 
  reframe(pop,
          origin, 
          shared_snps,
          shared_prop = shared_snps/sum(shared_snps)
            ) %>% 
  group_by(origin) %>% 
  summarise(full = mean(shared_prop)) %>% 
  pivot_longer(cols = 2, names_to="pop",values_to = "mean_prop")
  admix_prop_pop
  
  mean_admix_prop_pop <- snps_w_pop %>% 
    filter(chrom== '2', between(start, 46520806-500000, 46613842+500000)) %>% 
    group_by(name, pop, origin) %>%
    summarize(shared_snps = n()) %>%  
    ungroup() %>% 
    group_by(name) %>% 
    reframe(pop,
          origin, 
          shared_snps,
          shared_prop = shared_snps/sum(shared_snps)
            ) %>% 
  group_by(origin, pop) %>% 
  summarise(mean_prop = mean(shared_prop))
  
  mean_admix_pop <- bind_rows(admix_prop_pop, mean_admix_prop_pop)
  
  
  mean_admix_pop %>% filter(pop!="full") %>% 
  ggplot() +
  geom_col(aes(x=pop, y=mean_prop, fill=origin), position='dodge') +
  geom_hline(data=mean_admix_pop %>% filter(pop=='full'), aes(yintercept = mean_prop, color=origin)) +
  theme(
    axis.text.x = element_text(angle=45, hjust=1, size=6)
  )

```

> Do the same for EPAS1

```{r}
snps_w_pop <- snps %>% 
  left_join(archaic_individuals %>% select(name,pop), by=c("name" = "name")) %>% 
  mutate(pop=factor(pop, levels=unique(archaic_individuals$pop)))

admix_prop_pop <- snps_w_pop %>% 
  group_by(name, pop, origin) %>%
  summarize(shared_snps = n()) %>%  
  ungroup() %>% 
  group_by(name) %>% 
  reframe(pop,
          origin, 
          shared_snps,
          shared_prop = shared_snps/sum(shared_snps)
            ) %>% 
  group_by(origin) %>% 
  summarise(full = mean(shared_prop)) %>% 
  pivot_longer(cols = 2, names_to="pop",values_to = "mean_prop")
  admix_prop_pop
  
  mean_admix_prop_pop <- snps_w_pop %>% 
  group_by(name, pop, origin) %>%
  summarize(shared_snps = n()) %>%  
  ungroup() %>% 
  group_by(name) %>% 
  reframe(pop,
          origin, 
          shared_snps,
          shared_prop = shared_snps/sum(shared_snps)
            ) %>% 
  group_by(origin, pop) %>% 
  summarise(mean_prop = mean(shared_prop))
  
  mean_admix_pop <- bind_rows(admix_prop_pop, mean_admix_prop_pop)
  
  
  mean_admix_pop %>% filter(pop!="full") %>% 
  ggplot() +
  geom_col(aes(x=pop, y=mean_prop, fill=origin), position='dodge') +
  geom_hline(data=mean_admix_pop %>% filter(pop=='full'), aes(yintercept = mean_prop, color=origin)) +
  theme(
    axis.text.x = element_text(angle=45, hjust=1, size=6)
  )
```


## The SNP count distribution

> The the number of times each SNP is shared

```{r count-how-many-times-each-snp-is-shared-in-an-individual, eval=FALSE}

shared <- snps %>% 
  filter(origin != "human") %>% 
  group_by(chrom, start, origin) %>% 
  summarise(n_shared = n()) %>% 
  mutate(chrom=factor(chrom, levels=c(1:22,'X')))

head(shared)
summary(shared$n_shared) %>% as.array() %>% as.data.frame() %>% pivot_wider(names_from=Var1, values_from = Freq) %>% kable(format='latex')

# Make a density plot
plot_snp_counts <- shared %>% 
  ggplot(aes(x=n_shared, fill=origin)) +
  geom_bar(stat='density', position='dodge', width=1, color=NA) +
  #geom_density(aes(alpha=origin, color=origin)) +
  #ggtitle("Distribution of shared SNP positions based on inferred origin") +
  xlab("Number of times a SNP position is repeated") +
  ylab("Density") +
  coord_cartesian(xlim=c(1,150)) +
  theme(legend.position = c(0.7,0.7),
        legend.margin = margin(.1,.1,.1,.1),
        legend.box.margin = margin(.1,.1,.1,.1), 
        legend.background = element_blank()) +
  NULL
plot_snp_counts
```

```{r save-plot-snp-counts, eval=FALSE}
ggsave(plot_snp_counts, 
       path = "graphics/", 
       filename = "snp-counts-density-1_150.svg", width = 6.5, height = 2)
```

> Try to use a boxplot to visualize

```{r boxplot-snp-counts, eval=FALSE}

# Prepare for plotting jittered outliers on a boxplot
shared <- shared %>% 
  group_by(origin) %>% 
  mutate(outlier.high = n_shared > quantile(n_shared, .75) + 1.50*IQR(n_shared),
         outlier.low = n_shared < quantile(n_shared, .25) - 1.50*IQR(n_shared)) %>% 
  mutate(outlier.color = case_when(outlier.high ~ "firebrick4", 
                                   outlier.low ~"steelblue"))

# Make a boxplot (also not really worth showing)
shared %>% 
  ggplot(aes(x=origin, y=n_shared, fill=origin)) +
  #geom_boxplot(outlier.shape=NA) +
  #geom_jitter(color=shared$outlier.color, width=0.35, alpha=0.1) +
  geom_jitter(aes(color=origin), alpha=0.1)
  
# Calculate the mean shared number of snps across origin
shared %>% 
  group_by(origin) %>% 
  summarise(min = min(n_shared),
            mean = mean(n_shared),
            median = median(n_shared),
            max = max(n_shared),
            n = n(),
            sum(n_shared <= 5)/n,
            sum(n_shared <= 10)/n,
            sum(n_shared <= 50)/n,
            prop_above_50   = sum((n_shared > 50))/n) %>% 
  kable(format = 'latex', digits=2)
```


### Test: simulate the SNP distribution
```{r, eval=FALSE}
rnorm()
```


## Calculate the correlation/distribution based on archaic segments

```{r total-archaic-segments, eval=FALSE}
head(archaic_segments)
mean(archaic_segments$length)
summary(archaic_segments$length)

total_seg_ind <- archaic_segments %>% 
  group_by(name, pop, region) %>% 
  summarise(total_length = sum(length)) %>% 
  arrange(region)

summary(total_seg_ind$total_length)

plot_total_seg_ind <- total_seg_ind %>% 
  ggplot(aes(x=total_length, fill=region)) +
  geom_histogram(bins=50, color='black', linewidth=0.2) +
  geom_vline(aes(xintercept = median(total_seg_ind$total_length)), linewidth=.8, color='black') +
  annotate('text', x=median(total_seg_ind$total_length)*1.2, y=58, label="Median") +
  xlab("Total length") +
  ylab("Count") +
  scale_x_continuous(expand=c(0,0)) +
  scale_y_continuous(expand=c(0,0)) +
  coord_cartesian(ylim = c(0,60))

plot_total_seg_ind
```

```{r save-total-archaic-segments, eval=FALSE}
ggsave(plot_total_seg_ind, path="graphics/", filename="total_segment_distribution.svg", 
       width = 6.5, height = 2)

```

## Try to do hierarchical clustering on my homemade correlation matrix

```{r hclust-on-heatmap}

correlation_matrix <- overlap_pair_df %>% 
  mutate(correlation = 2*total_length/(total_archaic.x+total_archaic.y)) %>% 
  select(individual_1, individual_2, correlation) %>% 
  mutate(individual_1=factor(individual_1, levels=archaic_individuals$name), 
         individual_2=factor(individual_2, levels=archaic_individuals$name)) %>% 
  rbind(., transform(., individual_1 = individual_2, individual_2 = individual_1)) %>% 
  pivot_wider(names_from = individual_1, values_from = "correlation", values_fill = 1) %>% 
  column_to_rownames("individual_2") %>% 
  as.matrix()


distance_matrix <- as.dist(1-correlation_matrix)
hc <- hclust(distance_matrix)



library(pheatmap)

dim(correlation_matrix)

# Example usage with distance_matrix
pheatmap(correlation_matrix)
         #cluster_rows = TRUE, cluster_cols = TRUE, )
  
```


```{r epas-x-data}

pairwise_overlap_x <-  pairwise_overlap %>% 
  mutate(correlation = 2*shared_x/(total_archaic.x+total_archaic.y)) %>% 
  select(individual_1, individual_2, correlation) %>% 
  mutate(individual_1=factor(individual_1, levels=archaic_individuals$name), 
         individual_2=factor(individual_2, levels=archaic_individuals$name)) %>% 
  rbind(., transform(., individual_1 = individual_2, individual_2 = individual_1)) %>% 
  pivot_wider(names_from = individual_1, values_from = "correlation", values_fill = 1) %>% 
  column_to_rownames("individual_2") %>% 
  as.matrix()

pheatmap(pairwise_overlap_x)

pairwise_overlap_epas <-  pairwise_overlap %>% 
  mutate(correlation = 2*shared_epas/(total_archaic.x+total_archaic.y)) %>% 
  select(individual_1, individual_2, correlation) %>% 
  mutate(individual_1=factor(individual_1, levels=archaic_individuals$name), 
         individual_2=factor(individual_2, levels=archaic_individuals$name)) %>% 
  rbind(., transform(., individual_1 = individual_2, individual_2 = individual_1)) %>% 
  pivot_wider(names_from = individual_1, values_from = "correlation", values_fill = 1) %>% 
  column_to_rownames("individual_2") %>% 
  as.matrix()

pheatmap(pairwise_overlap_epas)

```

