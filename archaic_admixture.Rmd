...
pairwise_overlap <- read_csv("results.csv")
...
individual_order <- archaic_individuals %>% arrange(factor(region))

compmap_df <- pairwise_overlap %>%
  arrange(region_1) %>% 
  select(c(1:3)) %>% 
  pivot_wider(names_from = individual_2, values_from = total_length, values_fill = 0) %>%
...
compmap <- Heatmap(compmap_df, name = "Total overlap",
                   show_column_dend = FALSE, show_row_dend = FLASE,
...
# Do something a bit unheard of
test <- individual_order %>% group_by(region) %>% summarise(n=n())
annot <- c(rep(".", (test$n[1]-1)%/%2), test$region[1], rep(".", (test$n[1]-1)%/%2),
           rep(".", (test$n[2]-1)%/%2), test$region[2], rep(".", (test$n[2])%/%2),
           rep(".", (test$n[3])%/%2), test$region[3], rep(".", (test$n[3])%/%2),
           rep(".", (test$n[4])%/%2), test$region[4], rep(".", (test$n[4])%/%2),
           rep(".", (test$n[5])%/%2), test$region[5], rep(".", (test$n[5])%/%2))

compmap
```


```{r try-using-pheatmap}
